    ...
    <div class="row">
        <div class="col-md-3">
            <a href="{{ route('drivers.filter') . '?status=inapprove' }}">
                <info-box text="@lang('admin/general.Unapproved')" number="{{ $countOfInapproveDrivers }}" total="{{ $countOfTotalDrivers }} / " color="red"
                          icon="ion-record"></info-box>
            </a>
        </div>
        <div class="col-md-3">
            <a href="{{ route('drivers.filter') . '?status=offline' }}">
                <info-box text="@lang('admin/general.Offline')" number="{{ $countOfOfflineDrivers }}" total="{{ $countOfTotalDrivers }} / " color="orange"
                          icon="ion-record"></info-box>
            </a>
        </div>
        <div class="col-md-3">
            <a href="{{ route('drivers.filter') . '?status=onway' }}">
                <info-box text="@lang('admin/general.On way')" number="{{ $countOfOnWayDrivers }}" total="{{ $countOfTotalDrivers }} / " color="blue"
                          icon="ion-record"></info-box>
            </a>
        </div>
        <div class="col-md-3">
            <a href="{{ route('drivers.filter') . '?status=online' }}">
                <info-box text="@lang('admin/general.Online')" number="{{ $countOfOnlineDrivers }}" total="{{ $countOfTotalDrivers }} / " color="green"
                          icon="ion-record"></info-box>
            </a>
        </div>
    </div>
                        ...
                        <div class="box-tools">
                            ...
                            <div class="input-group input-group-sm" style="width: 150px;">
                                <input type="text" name="q" class="form-control pull-right"
                                       placeholder="@lang('admin/general.Search')" value="{{ Request::get('q') }}">
                        ...
                        @if (!$drivers->isEmpty())
                            <table class="table table-hover">
                                ...
                                @foreach($drivers as $driver)
                                    <tr onclick="window.document.location='{{ action('Admin\DriverController@show', ['id' => $driver->id]) }}';"
                                        style="cursor: pointer;">
                                        <td><img src="{{ $driver->getPicture() }}" alt="driver picture"
                                                 class="img-circle" width="24"></td>
